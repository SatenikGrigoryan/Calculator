class Calculator {
    ...
    generateResult() {
        if (this.getLastInputType() === "number") {
            const self = this;
            const simpifyExpression = function (currentExpression, operator) {
                if (currentExpression.indexOf(operator) === -1) {
                    ...
                    return simpifyExpression(currentExpression, operator);
                }
            }
            let result = ["x", "÷", "-", "+"].reduce(simpifyExpression, this.getAllInputValues());
